(function ($) {
    ...
    function quick_product_variable() {
        ...
        $(document).on('click', btnSelector, function (e) {
            ...
            if ($product.hasClass('quick-shop-loaded')) {
                ...
                $('body').trigger('liquory-quick-view-displayed');
                return;
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    // liquoryThemeModule.swatchesVariations();
            ...
            // Ajax action.
            xhr = $.ajax({
                url: wc_add_to_cart_params.wc_ajax_url.toString().replace('%%endpoint%%', 'liquory_add_to_cart'),
                data: formData,
                type: 'POST',
                processData: false,
                contentType: false,
    ...
    function sendRequest(url) {
        ...
        xhr = $.ajax({
            ...
            url: url,
            beforeSend: function () {
                var $products = $('ul.liquory-products');
                $products.addClass('preloader');
            },
            success: function (data) {
                let $html = $(data);
                $('#main ul.liquory-products').replaceWith($html.find('#main ul.liquory-products'));
                $('#main .woocommerce-pagination').replaceWith($html.find('#main .woocommerce-pagination'));
                window.history.pushState(null, null, url);
                xhr = false;
                $(document).trigger('liquory-products-loaded')
            }
        });
    }

    $body.on('change', '.liquory-products-per-page #per_page', function (e) {
        e.preventDefault();
        var url = this.value;
        sendRequest(url);
    });

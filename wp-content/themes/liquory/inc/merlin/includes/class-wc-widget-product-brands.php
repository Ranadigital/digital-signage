 ...
 * @extends WC_Widget
 */
class Liquory_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'liquory');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'liquory');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'liquory'),
				'label' => esc_html__('Title', 'liquory'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'liquory'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'liquory'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'liquory'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'liquory'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'liquory'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'liquory'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'liquory'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Liquory_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'liquory');
			...
			echo '<ul class="product-brands" id="liquory-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Liquory_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'liquory'),
		'singular_name'              => esc_html__('Brands', 'liquory'),
		'menu_name'                  => esc_html__('Brands', 'liquory'),
		'all_items'                  => esc_html__('All Brands', 'liquory'),
		'parent_item'                => esc_html__('Parent Brand', 'liquory'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'liquory'),
		'new_item_name'              => esc_html__('New Brand Name', 'liquory'),
		'add_new_item'               => esc_html__('Add New Brands', 'liquory'),
		'edit_item'                  => esc_html__('Edit Brand', 'liquory'),
		'update_item'                => esc_html__('Update Brand', 'liquory'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'liquory'),
		'search_items'               => esc_html__('Search Brands', 'liquory'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'liquory'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'liquory'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
